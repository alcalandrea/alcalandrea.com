...
import {loadAwarenessProjectImages} from "./functions"

export default async function BrandAwarenessImagePage({
  params: {projectId},
}: {
  params: {projectId: string}
}) {
  const images = await loadAwarenessProjectImages(projectId)
  if (images.length === 0) {
    redirect("/brand-awareness")
  }

  const route = `/brand-awareness#${projectId}`
